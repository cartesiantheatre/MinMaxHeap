  ...
  : m_LessThanComparison(LessThanComparison)
{
    ...
    /*
        N.B. The following call may trigger a core dump on GCC 10.2 when
             compiling with -D_GLIBCXX_DEBUG enabled. The core dump is in
             response to a false positive detection of an improper iterator
             dereference. This is reported upstream here:

             <https://gcc.gnu.org/bugzilla/show_bug.cgi?id=99402>

             Workaround is to use the subsequent version which accomplishes
             the same thing.
    */
    //std::copy(First, Last, std::begin(m_Heap));
    std::copy_n(First, SourceElementCount, std::begin(m_Heap));
...
void MinMaxHeap<Type, LessThanComparisonType>::DeleteMinimum()
{
    ...
    if(IsEmpty())
        throw std::out_of_range(_("Cannot delete minimum from an empty min-max heap."));
...
MinMaxHeap<Type, LessThanComparisonType>::FindMaximumOffset() const
{
    ...
    if(IsEmpty())
        throw std::out_of_range(_("Cannot find maximum in an empty min-max heap."));
...
MinMaxHeap<Type, LessThanComparisonType>::FindMinimumOffset() const
{
    ...
    if(IsEmpty())
        throw std::out_of_range(_("Cannot find minimum in an empty min-max heap."));
