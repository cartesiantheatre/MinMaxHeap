...
class MinMaxHeap
{
    /*
        Benchmarking:
            After integration into a CAD application that performs stress
            analyses on simulated physical structures, and after performing an
            analyses on a relative large physical structure, there was a 56 %
            savings in time. Before integration the running time took
            124,962 seconds, or about 34.7 hours. After integration this time
            was reduced down to 55,465 seconds, or about 15.4 hours.
    */
    ...
    protected:
        ...
        static int BinaryLogarithmIntegerFloor(UnsignedIntegralType Value) noexcept
        {
            ...
            //  generic implementation that follows...
                ...
                // And if no built-in available for given width, raise error...
                    ...
                    static_assert(false, "No appropriate __builtin_clz for given width.");
        ...
        {
            // Avoid code duplication by calling const version. This is done by
            //  safely casting away the const return type and adding const to
            //  *this so we can call the const version of the implementation...
            return const_cast<Type &>(std::as_const(*this).GetValue(
                GetParentOffset(GetParentOffset(GrandChildOffset))));
        }
        ...
        NodeOffsetType GetLastNodeOffset() const
        {
            ...
            if(m_Heap.empty())
                throw std::out_of_range(_("No last node in an already empty heap."));
        ...
        constexpr NodeOffsetType GetRootOffset() const noexcept
        {
            return 0;
        }
        ...
        Type &GetValue(const NodeOffsetType NodeOffset)
        {
            ...
            //  safely casting away the const return type and adding const to
            //  *this so we can call the const version of the implementation...
            return const_cast<Type &>(std::as_const(*this).GetValue(NodeOffset));
        }
